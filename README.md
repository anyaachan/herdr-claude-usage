...
- **Tab bar** (always visible): `✳ 5h 29% ↻2am · wk 18% · Fable 15% · € 87%`. A percentage above 80 gets a `!!` suffix (`BAR_HOT` in the plugin config); the tab bar renders no ANSI colors. `BAR_WARN` still drives the dashboard's amber bars. A trailing `⁺` means another signed-in profile is also active.
